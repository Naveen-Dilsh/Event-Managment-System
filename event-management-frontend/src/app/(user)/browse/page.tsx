...
import {
    ...
    Ticket, Users, ChevronRight, Sparkles, LogOut
} from "lucide-react";
            ...
            try {
                ...
                if (eventsData.status === "fulfilled") {
                    setEvents(eventsData.value.filter(e =>
                        e.status === "PUBLISHED" || e.status === "ONGOING" || e.status === "DRAFT"
                    ));
                }
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <div className="sticky top-0 z-40 border-b border-border/50 bg-background/80 backdrop-blur-md">
                <div className="mx-auto max-w-6xl flex h-14 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center gap-2 font-bold gradient-text">
                        <Sparkles className="h-4 w-4" /> EventFlow
                    </div>
                    <div className="flex items-center gap-3 sm:gap-4">
                        <a href="/my-bookings" className="text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:inline-block">My Bookings</a>
                        <a href="/my-tickets" className="text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:inline-block">My Tickets</a>
                        <a href="/my-loyalty" className="text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:inline-block">Loyalty</a>
                        <a href="/profile" className="text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:inline-block">Profile</a>
                        <button onClick={logout} className="flex items-center gap-1 text-sm font-medium text-red-400 hover:text-red-300 transition-colors">
